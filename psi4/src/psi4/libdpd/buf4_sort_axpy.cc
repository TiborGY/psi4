...
/* Init input and output buffers and read in all blocks of both */
    ...
    switch (index) {
        ...
        case pqsr:
            ...
            if (incore) {  // p->p; q->q; s->r; r->s = pqsr
                buf4_sort_pqsr_incore<true>(OutBuf, *InBuf, nirreps, my_irrep, alpha);
        ...
        case prqs:
            ...
            if (incore) {  // p->p; r->q; q->r; s->s = prqs
                buf4_sort_prqs_incore<true>(OutBuf, *InBuf, nirreps, my_irrep, alpha);
            } else {  // out-of-core pqrs <- prqs
                for (Gpq = 0; Gpq < nirreps; Gpq++) {
                    Grs = Gpq ^ my_irrep;
