...
void SAPT2p3::exch_ind30() {
    ...
    auto vAR = std::make_shared<Matrix>("vAR", noccA_, nvirA_);

    tAR->load(psio_, PSIF_SAPT_AMPS, Matrix::SaveType::SubBlocks);
    psio_->read_entry(PSIF_SAPT_AMPS, "AR Exch-Ind Integrals", (char *)vAR->get_pointer(), sizeof(double) * noccA_ * nvirA_);

    double ex_1 = -2.0 * C_DDOT(noccA_ * nvirA_, tAR->get_pointer(), 1, vAR->get_pointer(), 1);

    tAR.reset();

    auto tBS = std::make_unique<Matrix>("Ind30 uBS Amplitudes", noccB_, nvirB_);
    auto vBS = std::make_shared<Matrix>("vBS", noccB_, nvirB_);

    tBS->load(psio_, PSIF_SAPT_AMPS, Matrix::SaveType::SubBlocks);
    psio_->read_entry(PSIF_SAPT_AMPS, "BS Exch-Ind Integrals", (char *)vBS->get_pointer(), sizeof(double) * noccB_ * nvirB_);

    double ex_2 = -2.0 * C_DDOT(noccB_ * nvirB_, tBS->get_pointer(), 1, vBS->get_pointer(), 1);

    tBS.reset();

    auto sAR = std::make_shared<Matrix>("sAR", noccA_, nvirA_);
    double **sARp = sAR->pointer();
    auto sBS = std::make_shared<Matrix>("sBS", noccB_, nvirB_);
    double **sBSp = sBS->pointer();

    for (int a = 0; a < noccA_; a++) {
        for (int r = 0; r < nvirA_; r++) {
            sARp[a][r] = wBAR_[a][r] / (evalsA_[a] - evalsA_[r + noccA_]);
        }
    }

    for (int b = 0; b < noccB_; b++) {
        for (int s = 0; s < nvirB_; s++) {
            sBSp[b][s] = wABS_[b][s] / (evalsB_[b] - evalsB_[s + noccB_]);
        }
    }

    double ex_3 = exch_ind30_1(sARp, sBSp);
    double ex_4 = exch_ind30_2(sARp);
    double ex_5 = exch_ind30_3(sBSp);
...
double SAPT2p3::exch_ind30_1(double **sAR, double **sBS) {
    ...
    for (int a = 0, ar = 0; a < noccA_; a++) {
        for (int r = 0; r < nvirA_; r++, ar++) {
            energy -= 2.0 * sAR[a][r] * C_DDOT(noccB_ * nvirB_, vARBSp[ar], 1, sBS[0], 1);
        }
    }

    auto xAR = std::make_shared<Matrix>("xAR", noccA_, nvirA_);
    auto xBS = std::make_shared<Matrix>("xBS", noccB_, nvirB_);

    C_DGEMM('N', 'T', noccA_, nvirA_, noccB_, 1.0, sAB_[0], nmoB_, sAB_[noccA_], nmoB_, 0.0, xAR->get_pointer(), nvirA_);

    C_DGEMM('T', 'N', noccB_, nvirB_, noccA_, 1.0, sAB_[0], nmoB_, &(sAB_[0][noccB_]), nmoB_, 0.0, xBS->get_pointer(), nvirB_);

    auto yAR = std::make_shared<Matrix>("yAR", noccA_, nvirA_);
    auto yBS = std::make_shared<Matrix>("yBS", noccB_, nvirB_);

    double **B_p_AR = get_AR_ints(1);
    double **B_p_BS = get_BS_ints(1);

    C_DGEMV('n', noccA_ * nvirA_, ndf_ + 3, 1.0, &(B_p_AR[0][0]), ndf_ + 3, diagBB_, 1, 0.0, yAR->get_pointer(), 1);

    C_DGEMV('n', noccB_ * nvirB_, ndf_ + 3, 1.0, &(B_p_BS[0][0]), ndf_ + 3, diagAA_, 1, 0.0, yBS->get_pointer(), 1);

    energy += 8.0 * C_DDOT(noccA_ * nvirA_, xAR->get_pointer(), 1, sAR[0], 1) * C_DDOT(noccB_ * nvirB_, yBS->get_pointer(), 1, sBS[0], 1);
    energy += 8.0 * C_DDOT(noccA_ * nvirA_, yAR->get_pointer(), 1, sAR[0], 1) * C_DDOT(noccB_ * nvirB_, xBS->get_pointer(), 1, sBS[0], 1);
...
double SAPT2p3::exch_ind30_2(double **sAR) {
    ...
    auto ssRB = std::make_shared<Matrix>("ssRB", nvirA_, noccB_);

    C_DGEMM('T', 'N', nvirA_, noccB_, noccA_, 1.0, sAR[0], nvirA_, sAB_[0], nmoB_, 0.0, ssRB->get_pointer(), noccB_);
    ...
    for (int a = 0; a < noccA_; a++) {
        C_DGEMM('T', 'N', noccB_, ndf_ + 3, nvirA_, 1.0, ssRB->get_pointer(), noccB_, A_p_AR[a * nvirA_], ndf_ + 3, 0.0,
    ...
    C_DGEMM('N', 'N', noccA_, noccB_, nvirA_, 1.0, sAR[0], nvirA_, sAB_[noccA_], nmoB_, 0.0, xAB->get_pointer(), noccB_);

    C_DGEMM('N', 'T', noccA_, nvirA_, noccB_, 1.0, xAB->get_pointer(), noccB_, ssRB->get_pointer(), noccB_, 0.0, xAR->get_pointer(), nvirA_);

    C_DGEMV('n', noccA_ * nvirA_, ndf_ + 3, 1.0, A_p_AR[0], ndf_ + 3, diagBB_, 1, 0.0, yAR->get_pointer(), 1);

    energy += 4.0 * C_DDOT(noccA_ * nvirA_, xAR->get_pointer(), 1, yAR->get_pointer(), 1);

    auto E_p_AB = std::make_shared<Matrix>("E_p_AB", noccA_ * noccB_, ndf_ + 3);
    double **E_p_ABp = E_p_AB->pointer();
    auto C_p_BB = std::make_shared<Matrix>("C_p_BB", noccB_ * noccB_, ndf_ + 3);

    for (int a = 0; a < noccA_; a++) {
        C_DGEMM('T', 'N', noccB_, ndf_ + 3, nvirA_, 1.0, ssRB->get_pointer(), noccB_, A_p_AR[a * nvirA_], ndf_ + 3, 0.0,
                E_p_ABp[a * noccB_], ndf_ + 3);
    }

    C_DGEMM('T', 'N', noccB_, noccB_ * (ndf_ + 3), noccA_, 1.0, xAB->get_pointer(), noccB_, E_p_AB->get_pointer(), noccB_ * (ndf_ + 3), 0.0,
            C_p_BB->get_pointer(), noccB_ * (ndf_ + 3));

    energy -= 2.0 * C_DDOT(noccB_ * noccB_ * (ndf_ + 3), C_p_BB->get_pointer(), 1, B_p_BB[0], 1);

    auto xBB = std::make_shared<Matrix>("xBB", noccB_, noccB_);
...
double SAPT2p3::exch_ind30_3(double **sBS) {
    ...
    auto ssAS = std::make_shared<Matrix>("ssAS", noccA_, nvirB_);
    double **ssASp = ssAS->pointer();

    C_DGEMM('N', 'N', noccA_, nvirB_, noccB_, 1.0, sAB_[0], nmoB_, sBS[0], nvirB_, 0.0, ssAS->get_pointer(), nvirB_);
    ...
    for (int b = 0; b < noccB_; b++) {
        C_DGEMM('N', 'N', noccA_, ndf_ + 3, nvirB_, 1.0, ssAS->get_pointer(), nvirB_, B_p_BS[b * nvirB_], ndf_ + 3, 0.0, C_p_ABp[b],
                noccB_ * (ndf_ + 3));
    }

    for (int a = 0; a < noccA_; a++) {
        C_DGEMM('N', 'N', noccB_, ndf_ + 3, nvirB_, 1.0, sBS[0], nvirB_, A_p_AS[a * nvirB_], ndf_ + 3, 0.0,
                D_p_ABp[a * noccB_], ndf_ + 3);
    }
    ...
    C_DGEMM('N', 'T', noccA_, noccB_, nvirB_, 1.0, &(sAB_[0][noccB_]), nmoB_, sBS[0], nvirB_, 0.0, xAB->get_pointer(), noccB_);

    C_DGEMM('T', 'N', noccB_, nvirB_, noccA_, 1.0, xAB->get_pointer(), noccB_, ssASp[0], nvirB_, 0.0, xBS->get_pointer(), nvirB_);

    C_DGEMV('n', noccB_ * nvirB_, ndf_ + 3, 1.0, B_p_BS[0], ndf_ + 3, diagAA_, 1, 0.0, yBS->get_pointer(), 1);

    energy += 4.0 * C_DDOT(noccB_ * nvirB_, xBS->get_pointer(), 1, yBS->get_pointer(), 1);
    ...
    for (int b = 0; b < noccB_; b++) {
        C_DGEMM('N', 'N', noccA_, ndf_ + 3, nvirB_, 1.0, ssASp[0], nvirB_, B_p_BS[b * nvirB_], ndf_ + 3, 0.0,
                E_p_ABp[b * noccA_], ndf_ + 3);
    }

    C_DGEMM('N', 'N', noccA_, noccA_ * (ndf_ + 3), noccB_, 1.0, xABp[0], noccB_, E_p_ABp[0], noccA_ * (ndf_ + 3), 0.0,
            C_p_AAp[0], noccA_ * (ndf_ + 3));

    energy -= 2.0 * C_DDOT(noccA_ * noccA_ * (ndf_ + 3), C_p_AAp[0], 1, A_p_AA[0], 1);

    auto xAA = std::make_shared<Matrix>("xAA", noccA_, noccA_);
