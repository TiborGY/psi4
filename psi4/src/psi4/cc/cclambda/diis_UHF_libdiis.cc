...
/*! \file
    \ingroup CCLAMBDA
    \brief DIIS extrapolation for UHF Lambda amplitudes using libdiis
*/
...
/*
** diis_UHF_libdiis: DIIS extrapolation for UHF Lambda amplitudes using libdiis
**
...
**   - L1: LIA, Lia (2 components with different orbital spaces)
**   - L2: LIJAB, Lijab, LIjAb (3 components)
**
** Error vectors: R = L_new - L_old (computed using DPD operations)
**
** Key difference from ROHF: UHF uses different DPD index spaces for beta orbitals
**   - Alpha L1: (0, 1) - alpha occupied/virtual
**   - Beta L1:  (2, 3) - beta occupied/virtual (different from ROHF!)
**
...
**   - Cleaner, more maintainable code (~120 lines vs ~280 lines original)
**
** Based on successful ccenergy UHF DIIS migration (100% validated).
**
** Parameters:
...
void CCLambdaWavefunction::diis_UHF_libdiis(int iter, int L_irr) {
     ...
     *
     * NOTE: UHF uses different orbital space indices than ROHF for beta orbitals
     */

    // L1a (LIA) - Alpha singles (space indices: 0, 1)
    ...
    // L1b (Lia) - Beta singles (space indices: 2, 3 - UHF specific!)
    global_dpd_->file2_init(&L1b_new, PSIF_CC_LAMBDA, L_irr, 2, 3, "New Lia");
    global_dpd_->file2_init(&L1b_old, PSIF_CC_LAMBDA, L_irr, 2, 3, "Lia");
    global_dpd_->file2_copy(&L1b_new, PSIF_CC_OEI, "R1b_ia");
    global_dpd_->file2_init(&R1b, PSIF_CC_OEI, L_irr, 2, 3, "R1b_ia");
    global_dpd_->file2_axpy(&L1b_old, &R1b, -1.0, 0);
    global_dpd_->file2_close(&L1b_old);

    // L2aa (LIJAB) - Alpha-alpha doubles (space indices: 2, 7)
    ...
    // L2bb (Lijab) - Beta-beta doubles (space indices: 12, 17)
    global_dpd_->buf4_init(&L2bb_new, PSIF_CC_LAMBDA, L_irr, 12, 17, 12, 17, 0, "New Lijab");
    global_dpd_->buf4_init(&L2bb_old, PSIF_CC_LAMBDA, L_irr, 12, 17, 12, 17, 0, "Lijab");
    global_dpd_->buf4_copy(&L2bb_new, PSIF_CC_LAMBDA, "R2bb_ijab");
    global_dpd_->buf4_init(&R2bb, PSIF_CC_LAMBDA, L_irr, 12, 17, 12, 17, 0, "R2bb_ijab");
    global_dpd_->buf4_axpy(&L2bb_old, &R2bb, -1.0);
    global_dpd_->buf4_close(&L2bb_old);

    // L2ab (LIjAb) - Alpha-beta doubles (space indices: 22, 28)
    global_dpd_->buf4_init(&L2ab_new, PSIF_CC_LAMBDA, L_irr, 22, 28, 22, 28, 0, "New LIjAb");
    global_dpd_->buf4_init(&L2ab_old, PSIF_CC_LAMBDA, L_irr, 22, 28, 22, 28, 0, "LIjAb");
    global_dpd_->buf4_copy(&L2ab_new, PSIF_CC_LAMBDA, "R2ab_IjAb");
    global_dpd_->buf4_init(&R2ab, PSIF_CC_LAMBDA, L_irr, 22, 28, 22, 28, 0, "R2ab_IjAb");
