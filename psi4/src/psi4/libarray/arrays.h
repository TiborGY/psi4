...
class Array2d {
   ...
   public:
    ...
    void copy(double **a);
    // cdgesv: solve a linear equation via lapack (from occ)
    void cdgesv(Array1d* Xvec, int errcod);
    // gemm: matrix multiplication C = A * B (dfocc signature)
    void gemm(bool transa, bool transb, const Array2d *a, const Array2d *b, double alpha, double beta);
    // gemm: matrix multiplication (occ signature)
...
#endif  // _psi_src_lib_libarray_arrays_h_
